...
チャット UI は WebView 上の React アプリ、ネイティブ側との橋渡しは JavaScript Bridge で行います。モデル呼び出しは Codex の Responses スキーマ（ステートレス、`store:false`、`reasoning.encrypted_content` をインラインで返却）に準拠しています。
